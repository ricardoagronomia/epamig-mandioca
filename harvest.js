...
(function () {
  ...
  setTimeout(() => {
    ...
    function updateSampleCode() {
      // Só atualiza se não estiver editando um registro existente
      if (blockInput && plotInput && sampleInput && !currentHarvestId) {
        ...
        if (block && plot) {
          ...
          // Só atualiza se campo vazio ou só com prefixo antigo
    ...
    if (blockInput) {
      blockInput.addEventListener("change", updateSampleCode);
    }

    if (plotInput) {
      plotInput.addEventListener("change", updateSampleCode);
    }

    // Preencher na carga se já tiver bloco/parcela selecionados
    updateSampleCode();
  }, 50);
}
