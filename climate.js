...
(function () {
  ...
  function renderClimatePage(container) {
    ...
    container.innerHTML = `
      ...
      <!-- Entrada rápida -->
      <div class="card">
        <div style="font-size:14px; font-weight:600; color:#111827; margin-bottom:6px;">Registro diário de clima</div>
        <p style="font-size:13px; color:#6b7280; margin-bottom:10px;">
          Use os campos abaixo para lançar manualmente os dados diários.
        </p>
        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:flex-end;">
          <div style="flex:1 1 120px;"><label for="clDate">Data</label><input type="date" id="clDate"></div>
          <div style="flex:1 1 100px;"><label for="clRain">Precipitação (mm)</label><input type="number" id="clRain" step="0.1" min="0" placeholder="Ex. 12.5"></div>
          <div style="flex:1 1 100px;"><label for="clTmax">Temp. máxima (°C)</label><input type="number" id="clTmax" step="0.1" placeholder="Ex. 30.2"></div>
          <div style="flex:1 1 100px;"><label for="clTmin">Temp. mínima (°C)</label><input type="number" id="clTmin" step="0.1" placeholder="Ex. 18.7"></div>
          <div style="flex:1 1 100px;"><label for="clTmean">Temp. média (°C)</label><input type="number" id="clTmean" step="0.1" placeholder="Ex. 24.5"></div>
          <div style="flex:1 1 100px;"><label for="clRh">Umidade relativa (%)</label><input type="number" id="clRh" step="1" min="0" max="100" placeholder="Ex. 75"></div>
          <div style="flex:0 0 auto;">
            <button type="button" class="btn-primary" style="margin-top:4px; font-size:13px; padding:6px 12px;" onclick="saveClimateDailyRecord()" ${isVisitor ? "disabled" : ""}>Salvar registro diário</button>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="margin-top:16px; border-left:3px solid #059669;">
        <div style="font-size:14px; font-weight:600; color:#065f46; margin-bottom:6px;">📊 Importação em lote (Excel/CSV)</div>
        <p style="font-size:13px; color:#6b7280; margin-bottom:10px;">
          Importe múltiplos registros. O sistema reconhece automaticamente colunas com nomes variados.
        </p>
        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center;">
          <input type="file" id="climateFileInput" accept=".xlsx,.xls,.csv" style="flex:1 1 200px; padding:6px; border:1px solid #d1d5db; border-radius:6px; font-size:13px;" ${isVisitor ? "disabled" : ""}>
          <button type="button" class="btn-primary" style="font-size:13px; padding:6px 16px;" onclick="importClimateFile()" ${isVisitor ? "disabled" : ""}>Importar registros</button>
          <button type="button" class="btn-secondary" style="font-size:13px; padding:6px 16px;" onclick="downloadExampleTemplate()">Baixar modelo Excel</button>
        </div>
      ...
      <div class="card" style="margin-top:16px;">
        <div style="font-size:14px; font-weight:600; color:#065f46; margin-bottom:6px;">Resumo mensal e acumulado</div>
        <p style="font-size:13px; color:#6b7280; margin-bottom:8px;">
          Precipitação total por mês, acumulada no ano e médias de temperatura/umidade.
        </p>
        <div style="overflow-x:auto;">
          <table>
            ...
            <tbody id="climateMonthlyTableBody"></tbody>
          </table>
        </div>
      </div>
      ...
      <!-- Registros diários -->
      <div class="card" style="margin-top:16px;">
        <div style="font-size:14px; font-weight:600; color:#111827; margin-bottom:6px;">Registros diários de clima</div>
        <p style="font-size:13px; color:#6b7280; margin-bottom:8px;">
          Registros lançados, com opção de editar ou excluir.
        </p>
        <div style="overflow-x:auto;">
          <table>
            <thead>
            ...
            <tbody id="climateDailyTableBody"></tbody>
          </table>
        </div>
      </div>
    `;

    if (typeof loadClimateDailyReadings === "function") loadClimateDailyReadings();
    if (typeof loadClimateMonthlySummary === "function") loadClimateMonthlySummary();
    if (typeof loadClimateQuickSummary === "function") loadClimateQuickSummary();
  }
  ...
  window.saveClimateDailyRecord = async function saveClimateDailyRecord() {
    ...
    try {
      if (currentClimateEditId) {
        const { error } = await s.from("climate_daily").update(payload).eq("id", currentClimateEditId);
  ...
  window.loadClimateDailyReadings = async function loadClimateDailyReadings() {
    ...
    try {
      const { data, error } = await s
        .from("climate_daily")
        .select("id, date, rain_mm, tmax_c, tmin_c, tmean_c, rh_mean")  // ← CORRIGIDO: adicionado tmean_c
      ...
      if (!data || data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; font-size:13px; color:#6b7280;">Nenhum registro climático diário ainda.</td></tr>`;
        return;
      }
        ...
        <tr>
          ...
          <td>
            <div style="display:flex; gap:4px; justify-content:flex-end;">
              ${isVisitor ? `<span style="font-size:11px; color:#9ca3af;">Somente leitura</span>` : `
                <button type="button" class="btn-secondary" style="font-size:12px; padding:4px 8px;" onclick='openClimateDailyEdit(${JSON.stringify(row)})'>Editar</button>
                <button type="button" class="btn-danger" style="font-size:12px; padding:4px 8px;" onclick="confirmDeleteClimateDaily('${row.id}')">Excluir</button>
              `}
            </div>
          </td>
        </tr>
      ...
      tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#b91c1c;">Erro ao carregar registros.</td></tr>`;
    }
  };
  ...
  window.loadClimateMonthlySummary = async function loadClimateMonthlySummary() {
    ...
    try {
      ...
      const linhas = monthSlots.map((s) => {
        ...
        return `<tr><td>${nomesMeses[s.month]}</td><td>${rain}</td><td>${rainAcum}</td><td>${tmax}</td><td>${tmin}</td><td>${rh}</td></tr>`;
      }).join("");

      tbody.innerHTML = linhas;
    } catch (err) {
      console.error("Erro ao carregar resumo mensal:", err);
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:#b91c1c;">Erro ao carregar resumo mensal.</td></tr>`;
    }
  };
  ...
  window.loadClimateQuickSummary = async function loadClimateQuickSummary() {
    ...
    try {
      ...
      let rainSum = 0, rainCount = 0, tempSum = 0, tempCount = 0, rhSum = 0, rhCount = 0;
  ...
  window.confirmDeleteClimateDaily = async function confirmDeleteClimateDaily(id) {
    if (window.currentRole === "visitor") {
      alert("Visitantes não podem excluir registros.");
      return;
    }
    if (!id || typeof s === "undefined") return;

    try {
      const { error } = await s.from("climate_daily").delete().eq("id", id);
      if (error) throw error;
      if (typeof loadClimateDailyReadings === "function") loadClimateDailyReadings();
    } catch (err) {
  ...
  window.downloadExampleTemplate = function downloadExampleTemplate() {
    const templateData = [
      { date: "2026-01-28", rain_mm: 12.5, tmax_c: 30.2, tmin_c: 18.7, tmean_c: 24.5, rh_mean: 75 },
      { date: "2026-01-29", rain_mm: 0, tmax_c: 32.1, tmin_c: 19.5, tmean_c: 25.8, rh_mean: 68 }
    ];
